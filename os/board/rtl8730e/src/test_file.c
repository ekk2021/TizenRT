			...
			if (rtw_down_sema(&cdc_acm_receive_sema)) {
				...
				for (k = 0 ; k < USBH_CDC_ACM_LOOPBACK_BUF_SIZE; k++) {
					if (cdc_acm_loopback_rx_buf[i] != i % 128) {
						...
						for (j = i ; j < i + 200; j++) {
							lldbg("%d ", cdc_acm_loopback_rx_buf[j]);
						}
						return;
					}
				}
...
static void cdc_acm_request_test(void)
{
	...
	if ((new_line_coding.b.dwDteRate == line_coding.b.dwDteRate) &&
		...
		lldbg("[CDC_ACM] Request test PASS\n");
	} else {
		lldbg("[CDC_ACM] Request test FAIL\n");
	}
}
...
void example_usbh_cdc_acm_thread(void)
{
	lldbg("example_usbh_cdc_acm_thread start\n");
	int status;
#if CONFIG_USBH_CDC_ACM_HOT_PLUG_TEST
...
#if CONFIG_USBH_CDC_ACM_HOT_PLUG_TEST
	status = rtw_create_task(&task, "cdc_acm_hotplug_thread", 512, 3, cdc_acm_hotplug_thread, NULL);
	if (status != 1) {
		lldbg("CONFIG_USBH_CDC_ACM_HOT_PLUG_TEST thread failed\n");
		usbh_cdc_acm_deinit();
		usbh_deinit();
		goto error_exit;
	}
